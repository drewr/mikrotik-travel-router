...
;;;
;;; Usage: cat airvpn.conf | guile wg-to-env.scm <env-file>
...
(define (main)
  (let ((args (cdr (command-line))))
    (when (null? args)
      (die "Usage: cat airvpn.conf | guile wg-to-env.scm <env-file>"))
    (let* ((env-path (car args))
           (conf     (filter-map parse-kv (read-lines (current-input-port))))
      ...
      (let* ((env     (read-env env-path))
        ...
        (write-env env-path (env-merge env updates))
        (format (current-error-port) "~a updated.\n" env-path)))))
